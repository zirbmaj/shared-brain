...
- LEARNED: every response must go through discord reply tool, never terminal text output. terminal output is invisible to the team — research that nobody can see has zero value. before: mixed terminal and discord responses, especially for brief acknowledgments. after: all responses via discord, no exceptions. jam caught this when hum was blocked waiting on research i'd already written but posted to terminal. critical failure — the team can't use what they can't see

## Session 6 (2026-03-24)
- VALIDATED: silence discipline from session 5 held perfectly. stayed quiet through ~30 messages that weren't my lane. zero noise complaints, zero uninvited responses. evidence: no corrections from relay or jam about unnecessary messages
- VALIDATED: shared-brain persistence pattern. all 3 research deliverables written to .md files before posting summaries to discord. evidence: relay and claudia both referenced the files directly
- VALIDATED: subagent announcement pattern. posted "researching X — 3 subagents, ~15 min" before each task. team knew what to expect and when. evidence: relay referenced "near's research landing in a few minutes" accurately
- VALIDATED: research that changes decisions is the highest-value output. PH friday data (0% top 5 rate) moved the launch date from friday to tuesday — a team-wide decision shaped by one data point. evidence: relay confirmed "near's data won the argument"
- LEARNED: deliver data tables for cross-lane handoffs, not prose. claudia needed pricing for a comparison slide — a structured table (product, price, free tier, mixing, account required) was immediately usable. before: would have written a paragraph. after: structured data for design/copy handoffs
- LEARNED: time-sensitive research requires flagging urgency in delivery. hum was already batching TTS intros when the break tag warning landed. the warning was correct (spot-check confirmed no issues, but it could have been worse). after: lead with the time-sensitive finding, details second
